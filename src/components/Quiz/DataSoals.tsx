...
export default function UserDataPage() {
  ...
  useEffect(() => {
    if (searchTerm) {
      ...
      setFiltered(
        users.filter((u) => `${u.question}`.toLowerCase().includes(term))
      );
    } else {
      setFiltered([]);
    }
              ...
              {pageData.map((soal, index) => (
                ...
                >
                  ...
                  <div className="text-sm text-gray-700 mb-1">
                    <strong>Jenis:</strong> {soal.jenis_soal}
                  </div>
                  <div className="text-sm text-gray-700 mb-1">
                    <strong>Level:</strong> {soal.level_soal}
                  </div>
                  <ul className="text-sm text-gray-600 list-disc pl-5 mb-1">
                    <li>
                      <strong>A:</strong> {soal.a}
                    </li>
                    <li>
                      <strong>B:</strong> {soal.b}
                    </li>
                    <li>
                      <strong>C:</strong> {soal.c}
                    </li>
                    <li>
                      <strong>D:</strong> {soal.d}
                    </li>
                  </ul>
                  <div className="text-sm text-green-600 mb-2">
                    <strong>Jawaban:</strong> {soal.answer}
                  </div>
                  <button
                    onClick={() => setDetailUser(soal)}
              ...
              <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 backdrop-blur-sm transition-opacity duration-300">
                <div className="animate-fade-in-scale bg-white dark:bg-gray-800 p-6 rounded-lg shadow-xl w-full max-w-md relative">
                  ...
                  <div className="space-y-2 text-sm text-gray-700 dark:text-gray-200">
                    <p>
                      <strong>Pertanyaan:</strong> {detailUser.question}
                    </p>
                    <p>
                      <strong>A:</strong> {detailUser.a}
                    </p>
                    <p>
                      <strong>B:</strong> {detailUser.b}
                    </p>
                    <p>
                      <strong>C:</strong> {detailUser.c}
                    </p>
                    <p>
                      <strong>D:</strong> {detailUser.d}
                    </p>
                    <p>
                      <strong>Jawaban:</strong>{" "}
                      {detailUser.answer.toUpperCase()}
                    </p>
                    <p>
                      <strong>Pembahasan:</strong> {detailUser.explanation}
                    </p>
                  </div>
